...
def _run_one_config(layers_orig, layers_noised, sensitive_per_layer, bins=20, k_mdav=3) -> dict:
...
def run(config: dict, out_dir: str) -> None:
    ...
    for eps in epsilons:
        for s in seeds:
            ...
            res = _run_one_config(layers_orig, layers_noised, sensitive_per_layer, bins=bins)
    ...
    try:
        ...
        with open(csv_path, "w") as f:
            f.write("epsilon,seed,chi2,kl,mmd,rmse,wass1,empa_bias_bua,empa_bias_tda\n")
            for r in rows:
                f.write(f"{r['epsilon']},{r['seed']},{r['chi2']},{r['kl']},{r['mmd']},{r['rmse']},{r['wass1']},{r['empa_bias_bua']},{r['empa_bias_tda']}\n")
        print(f"Saved: {csv_path}")
    ...
    try:
        ...
        axes[0].semilogy(eps_list, [r["rmse"] for r in rows], "o-", color="C0", label="rMSE", markersize=8)
        axes[0].set_xlabel("Privacy budget $\\epsilon$")
        axes[0].set_ylabel("rMSE")
        axes[0].set_xscale("log")
        axes[0].legend()
        axes[0].grid(True, alpha=0.3)
        axes[1].semilogy(eps_list, [r["chi2"] for r in rows], "^-", color="C1", label="Chi-square", markersize=6)
        axes[1].semilogy(eps_list, [r["kl"] for r in rows], "s-", color="C2", label="K-L div.", markersize=6)
        axes[1].semilogy(eps_list, [r["mmd"] for r in rows], "d-", color="C3", label="MMD", markersize=6)

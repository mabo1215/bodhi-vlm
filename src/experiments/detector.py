...
def _collect_detr_features(model, processor, images: torch.Tensor, device: str) -> list:
    ...
    inp = processor(images=imgs_np, return_tensors="pt")
...
def _run_yolo(model_name: str, device: str, images: torch.Tensor, epsilon: float, seed: int):
    ...
    model.to(device)
    model.eval()
    feats_orig = _subsample_layers(_collect_yolo_features(model, images), seed=seed)
    feats_noised = _subsample_layers(_collect_yolo_features(model, _add_noise_to_images(images, epsilon, seed)), seed=seed)
